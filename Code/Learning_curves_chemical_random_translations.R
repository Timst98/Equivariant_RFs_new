...
grad_standard4=function(xtr,ytr,l1,sigma1,l2,sigma2,l3,sigma3){
  ...
  for(i in 1:(length(as.matrix(x1))/9)){
    for (j in 1:(length(as.matrix(x2))/9)){
      X1=x1[i,]-rep(x1[i,1:3],3);X2=x2[j,]-rep(x2[j,1:3],3)
      if(norm(X1[4:6])<norm(X1[7:9])){
        X1=c(x1[i,1:3],x1[i,7:9],x1[i,4:6])
      }else{
        X1=x1[i,]
      }
      if(norm(X2[4:6])<norm(X2[7:9])){
        X2=c(x2[j,1:3],x2[j,7:9],x2[j,4:6])
      }else{
        X2=x2[j,]
      }
      x1=x1[i,];x2=x2[j,]
...
for( i in 1:ntrain){
  ...
  }))+t(sapply(1:length(train_ind),function(i){
    a=runif(3,-4,4)
    rep(a,3)
  }))
  ...
  }))+t(sapply(1:length(test_ind),function(i){
    a=runif(3,-4,4)
    rep(a,3)
  }))
...
res=list(RMSES,LogS)
save(list = "res", file = paste0("learning_curves_chemical_random_translations_", id, ".rda"))
