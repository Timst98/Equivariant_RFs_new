...
cross_cov_mat3d_3=function(x1,x2,l1,sigma1,l2,sigma2,l3,sigma3){
  ...
  for(i in 1:(length(as.matrix(x1))/9)){
    for (j in 1:(length(as.matrix(x2))/9)){
      ...
      cov1= cov_mat3d_3(x1[i,],x2[j,],l1,sigma1,l2,sigma2,l3,sigma3)
...
if(complete_run){
RMSES=LogS=matrix(nrow=ntrain,ncol=4)
colnames(RMSES)<-c("standard","standard 2","standard 3","fundamental")
colnames(LogS)<-c("standard","standard 2","standard 3","fundamental")
}else{
RMSES=LogS=numeric(ntrain)
}
...
for( i in 1:ntrain){
  ...
  rot_ind=sample(10,length(train_ind),replace = TRUE)
  Xtr=t(sapply(1:length(train_ind),function(i){
    as.matrix(dat[[rot_ind[i]]][X_names])[-duplicates,][train_ind[i],]
  }))
  
  Ytr=t(sapply(1:length(train_ind),function(i){
    as.matrix(dat[[rot_ind[i]]][Y_names])[-duplicates,][train_ind[i],]
  }))
  
  rot_ind=sample(10,length(test_ind),replace = TRUE)
  Xte=t(sapply(1:length(test_ind),function(i){
    as.matrix(dat[[rot_ind[i]]][X_names])[-duplicates,][test_ind[i],]
  }))
  
  Yte=t(sapply(1:length(test_ind),function(i){
    as.matrix(dat[[rot_ind[i]]][Y_names])[-duplicates,][test_ind[i],]
  }))
  ...
  (RMSES[i,4]=rmse_eq=mean(apply((cbind(posterior_mean_eq[1:(length(posterior_mean_eq)/3)],
  ...
  (LogS[i,4]=LogS_eq=(t(c(Yte[,1],Yte[,2],Yte[,3])-posterior_mean_eq)%*%
  ...
  (RMSES[i,2]=rmse2=mean(apply((cbind(posterior_mean2[1:(length(posterior_mean)/3)],
  ...
  (LogS[i,2]=LogS2=(t(c(Yte[,1],Yte[,2],Yte[,3])-posterior_mean2)%*%
  ...
  Ktetr2=cross_cov_mat3d(Xte,Xtr,opt_par[1],
                         opt_par[2],
                         opt_par[3],
                         opt_par[4],
                         opt_par[5],
  ...
  Ktetr_trtr_inv2=Ktetr2%*%solve(cross_cov_mat3d(Xtr,Xtr,opt_par[1],
                                                 opt_par[2],
                                                 opt_par[3],
                                                 opt_par[4],
                                                 opt_par[5],
  ...
  posterior_cov2= cross_cov_mat3d(Xte,Xte,opt_par[1],
                                  opt_par[2],
                                  opt_par[3],
                                  opt_par[4],opt_par[5],
                                  opt_par[6])-
  ...
  Ktetr2=cross_cov_mat3d_3(Xte,Xtr,opt_par[1],
                           opt_par[2],
                           opt_par[3],
                           opt_par[4],
                           opt_par[5],
  ...
  Ktetr_trtr_inv2=Ktetr2%*%solve(cross_cov_mat3d_3(Xtr,Xtr,opt_par[1],
                                                   opt_par[2],
                                                   opt_par[3],
                                                   opt_par[4],
                                                   opt_par[5],
  ...
  (RMSES[i,3]=rmse2=mean(apply((cbind(posterior_mean2[1:(length(posterior_mean)/3)],
                                      posterior_mean2[(length(posterior_mean)/3+1):
                                                        (2*length(posterior_mean)/3)],
                                      posterior_mean2[(2*length(posterior_mean)/3+1):
                                                        (length(posterior_mean))])-Yte)^2,1,sum)
  ...
  posterior_cov2= cross_cov_mat3d_3(Xte,Xte,opt_par[1],
                                    opt_par[2],
                                    opt_par[3],
                                    opt_par[4],opt_par[5],
                                    opt_par[6])-
  ...
  (LogS[i,3]=LogS2=(t(c(Yte[,1],Yte[,2],Yte[,3])-posterior_mean2)%*%
                      solve(posterior_cov2
                            +diag(1e-6,3*nrow(Xte))
                      )%*%
                      (c(Yte[,1],Yte[,2],Yte[,3])-posterior_mean2)
  ...
  Ktetr2=cross_cov_mat3d_4(Xte,Xtr,opt_par[1],
                           opt_par[2],
                           opt_par[3],
                           opt_par[4],
                           opt_par[5],
  ...
  Ktetr_trtr_inv2=Ktetr2%*%solve(cross_cov_mat3d_4(Xtr,Xtr,opt_par[1],
                                                   opt_par[2],
                                                   opt_par[3],
                                                   opt_par[4],
                                                   opt_par[5],
  ...
  (RMSES[i,4]=rmse2=mean(apply((cbind(posterior_mean2[1:(length(posterior_mean)/3)],
                                      posterior_mean2[(length(posterior_mean)/3+1):
                                                        (2*length(posterior_mean)/3)],
                                      posterior_mean2[(2*length(posterior_mean)/3+1):
                                                        (length(posterior_mean))])-Yte)^2,1,sum)
  ...
  posterior_cov2= cross_cov_mat3d_4(Xte,Xte,opt_par[1],
                                    opt_par[2],
                                    opt_par[3],
                                    opt_par[4],opt_par[5],
                                    opt_par[6])-
  ...
  (LogS[i,4]=LogS2=(t(c(Yte[,1],Yte[,2],Yte[,3])-posterior_mean2)%*%
                      solve(posterior_cov2
                            +diag(1e-6,3*nrow(Xte))
                      )%*%
                      (c(Yte[,1],Yte[,2],Yte[,3])-posterior_mean2)
  ...
  Xtr=t(apply(Xtr,1,function(x){
    x1=x-rep(x[1:3],3)
    norm=function(x){sum(x^2)^.5}
    if(norm(x1[4:6])<norm(x1[7:9])){
      x=c(x[1:3],x[7:9],x[4:6])
    }
    x
  }))
  
  
  Xte=t(apply(Xte,1,function(x){
    x1=x-rep(x[1:3],3)
    norm=function(x){sum(x^2)^.5}
    if(norm(x1[4:6])<norm(x1[7:9])){
      x=c(x[1:3],x[7:9],x[4:6])
...
res=list(RMSES,LogS)
save(list = "res", file = paste0("learning_curves_chemical_just_swap_", id, ".rda"))
