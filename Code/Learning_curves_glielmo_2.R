...
RMSES=LogS=numeric(ntrain);OPT_PAR=numeric(ntrain)
...
opt_par=Adam(function(x) {
    ...
    grad_glielmo(Xtr, Ytr, x)
  },1,lb=1e-4,lr=lr,max_iter=max_iter)
  ...
  Ktetr_trtr_inv2=Ktetr2%*%solve(cross_glielmo_cov_mat(Xtr,Xtr,opt_par)+diag(jit,nrow=3*nrow(Xtr)))
  ...
  posterior_cov2= cross_glielmo_cov_mat(Xte,Xte,opt_par)-
...
save(list = "res", file = paste0("learning_curves_glielmo_2_", id, ".rda"))
