...
ntrain=10;x_values=seq(20,1000,l=ntrain)
...
X=t(sapply(1:nrow(rots),function(x){
  ...
  psi_3=matrix(c(cos(phi2),0,-sin(phi2),0,1,0,sin(phi2),0,cos(phi2)),
               nrow=3,byrow = 1)
  as.numeric(psi_3%*%psi_2%*%psi_1%*%matrix(X[x,],ncol=9)+matrix(rep(translation,9),ncol=9))
...
Xte=X[1501:min(nrow(X),1500+test_size),];Yte=Y[1501:min(nrow(X),1500+test_size),]
####################################################################
...
train_size_stepsize=c(10,rep(100,9))
...
save(list = "res", file = paste0("learning_curves_NMF_no_train_translations_long_", id, ".rda"))
