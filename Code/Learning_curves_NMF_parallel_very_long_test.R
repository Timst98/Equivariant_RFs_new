...
Xte=X[8001:min(nrow(X),8000+test_size),];Yte=Y[8001:min(nrow(X),8000+test_size),]
####################################################################
...
print(max_iter)  
  ...
  ntr=nrow(train_ind)
print(c(ntr,ntr/Batchsize))
print(dim(matrix(sample(1:nrow(Xtr)),ncol=ntr/Batchsize)))
  opt_par=Adam(function(x) {
    log_likelihood_standard(
      Ytr,Xtr, x[1], x[2])
  },function(x) {
    inds=matrix(sample(1:nrow(Xtr)),ncol=ntr/Batchsize)
    apply(foreach(ii=1:(ntr/Batchsize),.combine = 'cbind',
                  .export = Export,
