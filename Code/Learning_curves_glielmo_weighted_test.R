...
RMSES=LogS=numeric(ntrain);OPT_PAR=matrix(nrow=ntrain,ncol=3)
...
for( i in 1:ntrain){
  ...
  train_ind=sample(851,5)
test_ind=(1:851)[-train_ind]
  Xtr=X[train_ind,]
  Xte=X[test_ind,]
  Ytr=Y[train_ind,]
  Yte=Y[test_ind,]
...
opt_par=Adam(function(x) {
    ...
    grad_glielmo(Xtr, Ytr, x[1],x[2],x[3])
  },c(1,.5,.5),lb=1e-4,ub=c(Inf,5,5),lr=lr,max_iter=max_iter)


  OPT_PAR[i,]=opt_par
  Ktetr2=cross_glielmo_cov_mat(Xte,Xtr,
                         opt_par[1],opt_par[2],opt_par[3])

  Ktetr_trtr_inv2=Ktetr2%*%solve(cross_glielmo_cov_mat(Xtr,Xtr,opt_par[1],opt_par[2],opt_par[3])+diag(jit,nrow=3*nrow(Xtr)))
  ...
  posterior_cov2= cross_glielmo_cov_mat(Xte,Xte,opt_par[1],opt_par[2],opt_par[3])-
...
save(list = "res", file = paste0("learning_curves_glielmo_weighted_", id, ".rda"))
