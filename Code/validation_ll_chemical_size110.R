...
RMSES=LogS=numeric(1);Opt_par=matrix(ncol=1,nrow=6)
...
data_set_size=nrow(X)
train_ind=sample(data_set_size,train_size)
tic()
print(c(lr,max_iter,ntrain,nrow(X),Nsplit))
for( i in 1:1){
  
  #train_ind=c(sample((1:data_set_size)[-train_ind],10))
  test_ind=(1:data_set_size)[-train_ind]
...
toc()
res=list(RMSES,LogS,Opt_par)
save(list = "res", file = paste0("learning_curves_chemical_VAL_LL_size110_", id, ".rda"))
}
