...
fund_cov_mat_scalar=function(x1,x2,l,sigma){
  ...
  psi_3=matrix(c(cos(phi2),0,-sin(phi2),0,1,0,sin(phi2),0,cos(phi2)),
               nrow=3,byrow = 1)
  if(min(which(partition>=r2/r1))%%2==0){
    phi2=pi
    psi_4=matrix(c(cos(phi2),0,-sin(phi2),0,1,0,sin(phi2),0,cos(phi2)),
                 nrow=3,byrow = 1)
  }else{
    phi2=0
    psi_4=matrix(c(cos(phi2),0,-sin(phi2),0,1,0,sin(phi2),0,cos(phi2)),
  ...
  A1=c(r1,(psi_4%*%psi_3%*%x_H_tilde_2)[1],(psi_4%*%psi_3%*%x_H_tilde_2)[2])
  ...
  psi_3_2=matrix(c(cos(phi2_2),0,-sin(phi2_2),0,1,0,sin(phi2_2),0,cos(phi2_2)),
                 nrow=3,byrow = 1)
  if(min(which(partition>=r2_2/r1_2))%%2==0){
    phi2_2=pi
    psi_4_2=matrix(c(cos(phi2_2),0,-sin(phi2_2),0,1,0,sin(phi2_2),0,cos(phi2_2)),
                   nrow=3,byrow = 1)
  }else{
    phi2_2=0
    psi_4_2=matrix(c(cos(phi2_2),0,-sin(phi2_2),0,1,0,sin(phi2_2),0,cos(phi2_2)),
  ...
  A2=c(r1_2,(psi_4_2%*%psi_3_2%*%x_H2_tilde_2)[1],(psi_4_2%*%psi_3_2%*%x_H2_tilde_2)[2])
  
  sigma^2*exp(-(norm(A1-A2))^2/(2*l^2))*t(psi_4%*%psi_3%*%psi_2%*%psi_1)%*%psi_4_2%*%psi_3_2%*%psi_2_2%*%psi_1_2
...
grad_fund_scalar=function(xtr,ytr,l,sigma){
  ...
  for(i in 1:(length(as.matrix(x1))/9)){
    for (j in 1:(length(as.matrix(x2))/9)){
      ...
      cov1=sigma^2*exp(-(norm(A1-A2))^2/(2*l^2))/(l^3)*(norm(A1-A2))^2*t(psi_4%*%psi_3%*%psi_2%*%psi_1)%*%
        psi_4_2%*%psi_3_2%*%psi_2_2%*%psi_1_2
      
      cov2=2*sigma*exp(-(norm(A1-A2))^2/(2*l^2))*t(psi_4%*%psi_3%*%psi_2%*%psi_1)%*%
        psi_4_2%*%psi_3_2%*%psi_2_2%*%psi_1_2
...
RMSES=LogS=numeric(ntrain);Opt_par=list()
...
for( i in 1:ntrain){
  ...
  (RMSES[i]=rmse_eq=mean(apply((cbind(posterior_mean_eq[1:(length(posterior_mean_eq)/3)],
  ...
  (LogS[i]=LogS_eq=(t(c(Yte[,1],Yte[,2],Yte[,3])-posterior_mean_eq)%*%
...
save(list = "res", file = paste0("learning_curves_chemical_misspec_", id, ".rda"))
