...
opt_alpha_nlopt=opt_alpha_nlopt2=opt_alpha_helm_nlopt=opt_alpha_helm_nlopt2=ALPHA

train_ind=sample(564,50)
test_ind=-train_ind
...
for(i in 1:length(ALPHA)){
    ...
    F_te=t(apply(Xte,1,F))
    F_tr=t(apply(Xtr,1,F))
    ...
    (RMSES_nlopt[i,3]=rmse_mix=mean(apply((cbind(posterior_mean_mix[1:(0.5*length(posterior_mean_mix))],
    ...
    (LOGS_nlopt[i,3]=LOGS_nlopt_mix=(t(c(Yte[,1],Yte[,2])-posterior_mean_mix)%*%
    ...
    (LOGS_nlopt[i,4]=LOGS_nlopt_fund=(t(c(Yte[,1],Yte[,2])-posterior_mean_fund)%*%
    ...
    (RMSES_nlopt[i,5]=rmse_fund=mean(apply((cbind(
      posterior_mean_fund[1:(0.5*length(posterior_mean_fund))],
      posterior_mean_fund
      [(0.5*length(posterior_mean_fund)+1):(length(posterior_mean_fund))])
      -Yte)^2,1,sum))^.5)
    ...
    (LOGS_nlopt[i,5]=LogS_fund=(t(c(Yte[,1],Yte[,2])-posterior_mean_fund)%*%
                                  inv(posterior_cov_fund+diag(opt_par[3]^2,2*nrow(Xte)))%*%
                                  (c(Yte[,1],Yte[,2])-posterior_mean_fund)
                                +determinant(posterior_cov_fund+diag(opt_par[3]^2,2*nrow(Xte))
                                )$modulus[1]+
...
res=list(RMSES_nlopt,LOGS_nlopt,opt_alpha_nlopt,opt_alpha_nlopt2,
                opt_alpha_helm_nlopt,opt_alpha_helm_nlopt2) 

save(list = "res", file = paste0("Scores_gulf_eq_noise_size_100_", id, ".rda"))
