...
XX=X[seq(1,nrow(X),by=2),]

Eq_noise=(t(apply(XX,1,F)))


opt_par=c(1.3,2.32,.14,1.3,0.83)

Ktetr_fund=fund_cov_mat(X,XX,opt_par[1],
                        opt_par[2],
                        opt_par[4],
                        opt_par[5])

Ktetr_trtr_inv_fund=Ktetr_fund%*%inv(fund_cov_mat(XX,XX,opt_par[1],
                                                  opt_par[2],
                                                  opt_par[4],
                                                  opt_par[5])
                                     
                                     +diag(opt_par[3]^2,nrow=2*nrow(XX)))

posterior_mean_fund=Ktetr_trtr_inv_fund%*%c(Eq_noise[,1],Eq_noise[,2])

posterior_cov_fund= fund_cov_mat(X,X,opt_par[1],
                                 opt_par[2],
                                 opt_par[4],
                                 opt_par[5])-Ktetr_trtr_inv_fund%*%t(Ktetr_fund)
set.seed(1)
Eq_noise=posterior_mean_fund+chol(posterior_cov_fund+diag(opt_par[3]^2,nrow=2*nrow(X)))%*%rnorm(2*nrow(X))

Eq_noise=scale(cbind(Eq_noise[1:564],Eq_noise[565:1128]))
...
res=list(opt_alpha_nlopt2,opt_alpha_helm_nlopt2)
  

save(list = "res", file = paste0("Scores_gulf_eq_GP_noise_size_100_ADAM", id, ".rda"))
