...
while (i <= length(lines)) {
  ...
  if (grepl("X_DIPOLE_MP2", line)) {
    current_dipole[1] <- as.numeric(strsplit(line, " = ")[[1]][2])
  }
  if (grepl("Y_DIPOLE_MP2", line)) {
    current_dipole[2] <- as.numeric(strsplit(line, " = ")[[1]][2])
  }
  if (grepl("Z_DIPOLE_MP2", line)) {
    current_dipole[3] <- as.numeric(strsplit(line, " = ")[[1]][2])
    Y=rbind(Y, current_dipole)
  }
  
  # Extract atomic coordinates
  if (grepl("[A-Z]", line)) {
    elements <- unlist(strsplit(line, "\\s+"))
    atom_type <- elements[2]  # Element symbol
    coords <- as.numeric(elements[3:5])
    
    # Check if the atom type is in the order we need
    if (atom_type %in% c("C", "N", "O", "H")) {
      current_coords <- c(current_coords,coords)
      if(length(current_coords)==27){
        X=rbind(X,current_coords)
        current_coords=c()
      }
    }
  }
...
library(progress)
pb=progress_bar$new(total=nrow(A)^2)
...
for( i in 1:ntrain){
  ...
  opt_par=Adam(function(x) {
    log_likelihood_fund(
      Ytr,Xtr, x[1], x[2])
  },function(x) {grad_fund(Xtr, Ytr, x[1], x[2])},rep(1,2),lr=lr,max_iter = max_iter)
