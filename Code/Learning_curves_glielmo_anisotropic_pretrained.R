...
nsigma2=20
max1=2;max2=2
grid=expand.grid(seq(0,max1,l=nsigma1),seq(0,max2,l=nsigma2))
...
RMSES=LogS=numeric(ntrain);OPT_PAR=matrix(nrow=ntrain,ncol=2)
Initial_par=OPT_PAR
...
grid.likelihoods=apply(grid,1,function(x){log_likelihood_glielmo(Ytr,Xtr,x[1],x[2])})



initial_par=as.numeric(grid[which.min(grid.likelihoods),])

  
   Initial_par[i,]=initial_par
opt_par=Adam(function(x) {
    log_likelihood_glielmo(
      Ytr,Xtr,x[1],x[2])
  },function(x) {
    grad_glielmo(Xtr, Ytr, x[1],x[2])
  },initial_par,lb=1e-4,lr=lr,max_iter=max_iter)
  ...
  Ktetr_trtr_inv2=Ktetr2%*%solve(cross_glielmo_cov_mat(Xtr,Xtr,opt_par[1],opt_par[2])+diag(jit,nrow=3*nrow(Xtr)))
  ...
  posterior_cov2= cross_glielmo_cov_mat(Xte,Xte,opt_par[1],opt_par[2])-
  ...
  (LogS[i]=LogS2=(t(c(Yte[,1],Yte[,2],Yte[,3])-posterior_mean2)%*%
                  ...
                  #  +determinant(posterior_cov2
                   #              +diag(1e-4,3*nrow(Xte))
                    #)$modulus[1]+
log(det(posterior_cov2 +diag(1e-4,3*nrow(Xte))))+
...
res=list(RMSES,LogS,OPT_PAR,Initial_par)
save(list = "res", file = paste0("learning_curves_glielmo_anisotropic_pretrained_", id, ".rda"))
