...
RMSES=LogS=matrix(ncol=2,nrow=ntrain);OPT_PAR=matrix(nrow=ntrain,ncol=3)
...
opt_par=Adam(function(x) {
  ...
  },rep(1,2),lr=lr,max_iter=max_iter,lb=1e-4)
...
opt_par=Adam(function(x) {
    log_likelihood_fund_scalar(
      Ytr,Xtr, x[1], x[2])
  },function(x) {grad_fund_scalar(Xtr, Ytr, x[1], x[2])},rep(1,2),lr=lr,max_iter = max_iter,lb=1e-4)
...
res=list(RMSES,LogS)
save(list = "res", file = paste0("learning_curves_glielmo_sanitycheck_", id, ".rda"))
