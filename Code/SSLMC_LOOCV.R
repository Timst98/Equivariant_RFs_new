...
for (j in 1:length(train_inds)){
  ...
  (LogS_standard[j]=(t(c(Yte)-posterior_mean_standard)%*%solve(posterior_cov_standard+
                                                                             diag(opt_par[5]^2,2))%*%
                       (c(Yte)-posterior_mean_standard)
                     +determinant(posterior_cov_standard+diag(opt_par[5]^2,2))$modulus[1]+
                       log(2*pi)*1)/1)
  ...
  for (i in 1:nrow(NU)){
    ...
    (LogS_matern[i,j]=(t(c(Yte)-posterior_mean_matern)%*%solve(posterior_cov_matern+
                                                                             diag(opt_tau^2,2))%*%
                         (c(Yte)-posterior_mean_matern)
                       +determinant(posterior_cov_matern+diag(opt_tau^2,2*1))$modulus[1]+
                         log(2*pi)))
...
save(list = "res", file = paste0("SS_LMC_LOOCV", id, ".rda"))
