...
rotation_onto_z=function(x){
    theta=atan2(x[1],x[2])
    xhat=matrix(c(cos(theta),-sin(theta),0,sin(theta),cos(theta),0,0,0,1),
                nrow=3,byrow = 1)%*%x
    
    gamma=atan2(xhat[2],xhat[3])
    return(matrix(c(1,0,0,0,cos(gamma),-sin(gamma),0,sin(gamma),cos(gamma)),
                  nrow=3,byrow = 1)%*%
               matrix(c(cos(theta),-sin(theta),0,sin(theta),cos(theta),0,0,0,1),
                      nrow=3,byrow = 1))
}
...
glielmo_3d_kernel=function(x1,x2,sigma){
    norm=function(x){sum(x^2)^.5}
    L=(2*sigma*sqrt(pi))^3
    K=matrix(0,nrow=3,ncol=3)
    for(i in 1:3){
        atom1_pos_i=x1[(3*(i-1)+1):(3*i)]
        r_i=norm(atom1_pos_i)
        for( j in 1:3){
            atom2_pos_j=x2[(3*(j-1)+1):(3*j)]
            r_j=norm(atom2_pos_j)
            gamma_ij=r_i*r_j/(2*sigma^2)
            alpha_ij=(r_i^2+r_j^2)/(4*sigma^2)
            phi_ij=exp(-alpha_ij)/(gamma_ij^2)*(gamma_ij*cosh(gamma_ij)-sinh(gamma_ij))
            K=K+t(rotation_onto_z(atom1_pos_i))%*%cbind(rep(0,3),rep(0,3),c(0,0,phi_ij))%*%
                rotation_onto_z(atom2_pos_j)
        }
    }
    return(K/L)
}
...
glielmo_3d_kernel_derivative=function(x1,x2,sigma){
    norm=function(x){sum(x^2)^.5}
    L=(2*sigma*sqrt(pi))^3
    L_prime=3*sigma^2*(2*sqrt(pi))^3
    K=K_prime=matrix(0,nrow=3,ncol=3)
    for(i in 1:3){
        atom1_pos_i=x1[(3*(i-1)+1):(3*i)]
        r_i=norm(atom1_pos_i)
        for( j in 1:3){
            ...
            r_j=norm(atom2_pos_j)
            gamma_ij=r_i*r_j/(2*sigma^2)
            gamma_ij_prime=-r_i*r_j/(sigma^3)
            alpha_ij=(r_i^2+r_j^2)/(4*sigma^2)
            alpha_ij_prime=-(r_i^2+r_j^2)/(2*sigma^3)
            cosh_prime=sinh(gamma_ij)*gamma_ij_prime
            sinh_prime=cosh(gamma_ij)*gamma_ij_prime
            phi_ij=exp(-alpha_ij)/(gamma_ij^2)*(gamma_ij*cosh(gamma_ij)-sinh(gamma_ij))
            
            phi_ij_prime=phi_ij*(-alpha_ij_prime)-2*phi_ij/(gamma_ij)*gamma_ij_prime+
                exp(-alpha_ij)/(gamma_ij^2)*(gamma_ij_prime*cosh(gamma_ij)+gamma_ij*
                                                 cosh_prime-sinh_prime)
            K=K+t(rotation_onto_z(atom1_pos_i))%*%cbind(rep(0,3),rep(0,3),c(0,0,phi_ij))%*%
                rotation_onto_z(atom2_pos_j)
            K_prime=K_prime+t(rotation_onto_z(atom1_pos_i))%*%cbind(rep(0,3),rep(0,3),c(0,0,phi_ij_prime))%*%
                rotation_onto_z(atom2_pos_j)
        }
    }
    return(K_prime/L-K/(L^2)*L_prime)
}

cross_glielmo_cov_mat=function(x1,x2,sigma){
    cov= matrix(0, ncol =3 * ifelse(length(as.matrix(x2)) == 9, 1,nrow(x2)),
                nrow = 3 * ifelse(length(as.matrix(x1)) == 9, 
                                  1, nrow(x1)))
    ...
    for(i in 1:(length(as.matrix(x1))/9)){
        for (j in 1:(length(as.matrix(x2))/9)){
            if(length(as.matrix(x2)) == 9){
                if(length(as.matrix(x1)) == 9){
                    cov1= glielmo_3d_kernel(x1,x2,sigma)
                    
                }else{
                    cov1= glielmo_3d_kernel(x1[i,],x2,sigma)
                    
                }
            }else if(length(as.matrix(x1)) == 9){
                cov1= glielmo_3d_kernel(x1,x2[j,],sigma)
                
            }else{
                cov1= glielmo_3d_kernel(x1[i,],x2[j,],sigma)
...
log_likelihood_glielmo=function(ytr,xtr,sigma){
    if(length(ytr)>3){
        if(ncol(ytr)==3){
            ytr=c(ytr[,1],ytr[,2],ytr[,3])
        }
    ...
    ntr=ifelse(length(xtr)==9,1,nrow(xtr))
    Ktr=cross_glielmo_cov_mat(xtr,xtr,sigma)+
        diag(jit,nrow=3*ntr)
...
grad_glielmo=function(xtr,ytr,sigma){
    Trace=function(x){sum(diag(x))}
    x1=x2=xtr
    norm=function(x){sum(x^2)^.5}
    
    K=cross_glielmo_cov_mat(xtr,xtr,sigma)+
    ...
    for(i in 1:(length(as.matrix(x1))/9)){
        for (j in 1:(length(as.matrix(x2))/9)){
            ...
            cov2=glielmo_3d_kernel_derivative(x1,x2,sigma)
            x1=x2=xtr
...
glielmo_3d_kernel=function(x1,x2,sigma){
  norm=function(x){sum(x^2)^.5}
  L=(2*sigma*sqrt(pi))^3
  x1=c(x1[4:6]-x1[1:3],x1[7:9]-x1[1:3])
  x2=c(x2[4:6]-x2[1:3],x2[7:9]-x2[1:3])
  K=matrix(0,nrow=3,ncol=3)
  for(i in 1:2){
    atom1_pos_i=x1[(3*(i-1)+1):(3*i)]
    r_i=norm(atom1_pos_i)
...
glielmo_3d_kernel=function(x1,x2,sigma,w1,w2){
  norm=function(x){sum(x^2)^.5}
  w=c(w1,w2)
  L=(sigma*sqrt(2*pi))^3

 x1=c(x1[4:6]-x1[1:3],x1[7:9]-x1[1:3])
  if(norm(x1[1:3])<norm(x1[4:6])){
    x1=x1[c(4:6,1:3)]
  }
 
  x2=c(x2[4:6]-x2[1:3],x2[7:9]-x2[1:3])
  if(norm(x2[1:3])<norm(x2[4:6])){
    x2=x2[c(4:6,1:3)]
  }
  ...
  for(i in 1:2){
    ...
    for( j in 1:2){
      ...
      alpha_ij=(r_i^2+r_j^2)/(4*sigma^2)
      phi_ij=exp(-alpha_ij)/(gamma_ij^2)*(gamma_ij*cosh(gamma_ij)-sinh(gamma_ij))
      K=K+w[i]*w[j]*t(rotation_onto_z(atom1_pos_i))%*%cbind(rep(0,3),rep(0,3),c(0,0,phi_ij))%*%
        rotation_onto_z(atom2_pos_j)
    }
  }
  return(K/L)
...
glielmo_3d_kernel_derivative=function(x1,x2,sigma,w1,w2){
  norm=function(x){sum(x^2)^.5}
  L=(sigma*sqrt(2*pi))^3
  L_prime=3*sigma^2*(sqrt(2*pi))^3
 x1=c(x1[4:6]-x1[1:3],x1[7:9]-x1[1:3])
  if(norm(x1[1:3])<norm(x1[4:6])){
    x1=x1[c(4:6,1:3)]
  }
  ...
  w=c(w1,w2)
  phi=matrix(nrow=2,ncol = 2)
  K=K_prime_sigma=K_prime_w1=K_prime_w2=matrix(0,nrow=3,ncol=3)
  for(i in 1:2){
    atom1_pos_i=x1[(3*(i-1)+1):(3*i)]
    r_i=norm(atom1_pos_i)
    for( j in 1:2){
      ...
      K=K+w[i]*w[j]*t(rotation_onto_z(atom1_pos_i))%*%cbind(rep(0,3),rep(0,3),c(0,0,phi_ij))%*%
        rotation_onto_z(atom2_pos_j)
      K_prime_sigma=K_prime_sigma+w[i]*w[j]*t(rotation_onto_z(atom1_pos_i))%*%cbind(rep(0,3),rep(0,3),c(0,0,phi_ij_prime))%*%
        rotation_onto_z(atom2_pos_j)
      if(i==1){
        if(j==1){
          K_prime_w1=K_prime_w1+ 2*w1*t(rotation_onto_z(atom1_pos_i))%*%cbind(rep(0,3),rep(0,3),c(0,0,phi_ij))%*%
            rotation_onto_z(atom2_pos_j)
          
        }else{
          K_prime_w1=K_prime_w1+ w2*t(rotation_onto_z(atom1_pos_i))%*%cbind(rep(0,3),rep(0,3),c(0,0,phi_ij))%*%
            rotation_onto_z(atom2_pos_j)
          K_prime_w2=K_prime_w2+ w1*t(rotation_onto_z(atom1_pos_i))%*%cbind(rep(0,3),rep(0,3),c(0,0,phi_ij))%*%
            rotation_onto_z(atom2_pos_j)
        ...
        if(j==1){
          ...
          K_prime_w2=K_prime_w2+ 2*w2*t(rotation_onto_z(atom1_pos_i))%*%cbind(rep(0,3),rep(0,3),c(0,0,phi_ij))%*%
            rotation_onto_z(atom2_pos_j)
        }
      }
  ...
  K_prime_sigma=K_prime_sigma/L-K/(L^2)*L_prime
  
  return(list(K_prime_sigma=K_prime_sigma,K_prime_w1=K_prime_w1/L,K_prime_w2=K_prime_w2/L))
}



## Cross covariance and likelihood

cross_glielmo_cov_mat=function(x1,x2,sigma,w1,w2){
  cov= matrix(0, ncol =3 * ifelse(length(as.matrix(x2)) == 9, 1,nrow(x2)),
              nrow = 3 * ifelse(length(as.matrix(x1)) == 9, 
                                1, nrow(x1)))
  ...
  for(i in 1:(length(as.matrix(x1))/9)){
    for (j in 1:(length(as.matrix(x2))/9)){
      if(length(as.matrix(x2)) == 9){
        if(length(as.matrix(x1)) == 9){
          cov1= glielmo_3d_kernel(x1,x2,sigma,w1,w2)
          
        }else{
          cov1= glielmo_3d_kernel(x1[i,],x2,sigma,w1,w2)
          
        }
      }else if(length(as.matrix(x1)) == 9){
        cov1= glielmo_3d_kernel(x1,x2[j,],sigma,w1,w2)
        
      }else{
        cov1= glielmo_3d_kernel(x1[i,],x2[j,],sigma,w1,w2)
...
log_likelihood_glielmo=function(ytr,xtr,sigma,w1,w2){
  if(length(ytr)>3){
    if(ncol(ytr)==3){
      ytr=c(ytr[,1],ytr[,2],ytr[,3])
    }
  ...
  ntr=ifelse(length(xtr)==9,1,nrow(xtr))
  Ktr=cross_glielmo_cov_mat(xtr,xtr,sigma,w1,w2)+
    diag(jit,nrow=3*ntr)
...
grad_glielmo=function(xtr,ytr,sigma,w1,w2){
  Trace=function(x){sum(diag(x))}
  x1=x2=xtr
  norm=function(x){sum(x^2)^.5}
  
  K=cross_glielmo_cov_mat(xtr,xtr,sigma,w1,w2)+
    diag(jit,nrow=3*nrow(xtr))
  ...
  for(i in 1:(length(as.matrix(x1))/9)){
    for (j in 1:(length(as.matrix(x2))/9)){
      x1=xtr[i,]
      x2=xtr[j,]
      deriv=glielmo_3d_kernel_derivative(x1,x2,sigma,w1,w2)
      cov1=deriv$K_prime_sigma
      cov2=deriv$K_prime_w1
      cov3=deriv$K_prime_w2
      x1=x2=xtr
      
      
      
      K_sigma[i, j] = cov1[1,1]
      K_sigma[ifelse(length(as.matrix(x1))==9,1,nrow(x1)) + i,
              ifelse(length(as.matrix(x2))==9,1,nrow(x2)) + j] = cov1[2,2]
      ...
      K_sigma[ifelse(length(as.matrix(x1))==9,1,nrow(x1)) + i, j] = cov1[2,1]
      K_sigma[2*ifelse(length(as.matrix(x1))==9,1,nrow(x1)) + i, j] = cov1[3,1]
      
      K_sigma[i,ifelse(length(as.matrix(x2))==9,1,nrow(x2)) + j] = cov1[1,2]
      K_sigma[i,2*ifelse(length(as.matrix(x2))==9,1,nrow(x2)) + j] = cov1[1,3]
      ...
      K_w1[i, j] = cov2[1,1]
      K_w1[ifelse(length(as.matrix(x1))==9,1,nrow(x1)) + i,
              ifelse(length(as.matrix(x2))==9,1,nrow(x2)) + j] = cov2[2,2]
      
      K_w1[2*ifelse(length(as.matrix(x1))==9,1,nrow(x1)) + i,
              2*ifelse(length(as.matrix(x2))==9,1,nrow(x2)) + j] = cov2[3,3]
      
      K_w1[ifelse(length(as.matrix(x1))==9,1,nrow(x1)) + i, j] = cov2[2,1]
      K_w1[2*ifelse(length(as.matrix(x1))==9,1,nrow(x1)) + i, j] = cov2[3,1]
      
      K_w1[i,ifelse(length(as.matrix(x2))==9,1,nrow(x2)) + j] = cov2[1,2]
      K_w1[i,2*ifelse(length(as.matrix(x2))==9,1,nrow(x2)) + j] = cov2[1,3]
      ...
      K_w2[i, j] = cov3[1,1]
      K_w2[ifelse(length(as.matrix(x1))==9,1,nrow(x1)) + i,
           ifelse(length(as.matrix(x2))==9,1,nrow(x2)) + j] = cov3[2,2]
      
      K_w2[2*ifelse(length(as.matrix(x1))==9,1,nrow(x1)) + i,
           2*ifelse(length(as.matrix(x2))==9,1,nrow(x2)) + j] = cov3[3,3]
      
      K_w2[ifelse(length(as.matrix(x1))==9,1,nrow(x1)) + i, j] = cov3[2,1]
      K_w2[2*ifelse(length(as.matrix(x1))==9,1,nrow(x1)) + i, j] = cov3[3,1]
      
      K_w2[i,ifelse(length(as.matrix(x2))==9,1,nrow(x2)) + j] = cov3[1,2]
      K_w2[i,2*ifelse(length(as.matrix(x2))==9,1,nrow(x2)) + j] = cov3[1,3]
  ...
  grad=0.5*c(
    -t(c(ytr[,1],ytr[,2],ytr[,3]))%*%inv_K%*%K_sigma%*%inv_K%*%c(
      ytr[,1],ytr[,2],ytr[,3])+Trace(inv_K%*%K_sigma),
    -t(c(ytr[,1],ytr[,2],ytr[,3]))%*%inv_K%*%K_w1%*%inv_K%*%c(
      ytr[,1],ytr[,2],ytr[,3])+Trace(inv_K%*%K_w1),
    -t(c(ytr[,1],ytr[,2],ytr[,3]))%*%inv_K%*%K_w2%*%inv_K%*%c(
      ytr[,1],ytr[,2],ytr[,3])+Trace(inv_K%*%K_w2)
  )
...
# Adam optimizer
glielmo_3d_kernel=function(x1,x2,sigma1,sigma2){
  norm=function(x){sum(x^2)^.5}
  ...
  for(i in 1:2){
    ...
    r_i=norm(atom1_pos_i)
    for( j in 1:2){
      sig=sum(sigma[c(i,j)]^2)
      L=(2*sig*pi)^1.5
      atom2_pos_j=x2[(3*(j-1)+1):(3*j)]
      r_j=norm(atom2_pos_j)
      gamma_ij=r_i*r_j/(2*sig)
      alpha_ij=(r_i^2+r_j^2)/(4*sig)
      phi_ij=exp(-alpha_ij)/(gamma_ij^2)*(gamma_ij*cosh(gamma_ij)-sinh(gamma_ij))
      K=K+1/L*t(rotation_onto_z(atom1_pos_i))%*%cbind(rep(0,3),rep(0,3),c(0,0,phi_ij))%*%
        rotation_onto_z(atom2_pos_j)
    }
  }
  return(K)
...
glielmo_3d_kernel_derivative=function(x1,x2,sigma1,sigma2){
  norm=function(x){sum(x^2)^.5}
  ...
  K_prime_sigma1=K_prime_sigma2=matrix(0,nrow=3,ncol=3)
  for(i in 1:2){
    atom1_pos_i=x1[(3*(i-1)+1):(3*i)]
    r_i=norm(atom1_pos_i)
    for( j in 1:2){
      ...
      if(i==1){
        if(j==1){
          alpha_ij_prime=-(r_i^2+r_j^2)/(4*sig^2)*4*sigma1
          gamma_ij_prime=-r_i*r_j/(2*sig^2)*4*sigma1
          ...
          K_prime_sigma1=K_prime_sigma1-1.5*(4*sigma1)*((sig)^(-2.5)*(2*pi)^(-1.5))*
            t(rotation_onto_z(atom1_pos_i))%*%cbind(rep(0,3),rep(0,3),c(0,0,phi_ij))%*%
            rotation_onto_z(atom2_pos_j)+1/L*t(rotation_onto_z(atom1_pos_i))%*%
            cbind(rep(0,3),rep(0,3),c(0,0,phi_ij_prime))%*%
            rotation_onto_z(atom2_pos_j)
          
          
          
        }else{
          alpha_ij_prime=-(r_i^2+r_j^2)/(4*sig^2)*2*sigma1
          gamma_ij_prime=-r_i*r_j/(2*sig^2)*2*sigma1
          ...
          K_prime_sigma1=K_prime_sigma1- 1.5*(2*sigma1)*((sig)^(-2.5)*(2*pi)^(-1.5))*
            t(rotation_onto_z(atom1_pos_i))%*%cbind(rep(0,3),rep(0,3),c(0,0,phi_ij))%*%
            rotation_onto_z(atom2_pos_j)+1/L*t(rotation_onto_z(atom1_pos_i))%*%
            cbind(rep(0,3),rep(0,3),c(0,0,phi_ij_prime))%*%
            rotation_onto_z(atom2_pos_j)
          ...
          alpha_ij_prime=-(r_i^2+r_j^2)/(4*sig^2)*2*sigma2
          gamma_ij_prime=-r_i*r_j/(2*sig^2)*2*sigma2
          ...
          K_prime_sigma2=K_prime_sigma2- 1.5*(2*sigma2)*((sig)^(-2.5)*(2*pi)^(-1.5))*
            t(rotation_onto_z(atom1_pos_i))%*%cbind(rep(0,3),rep(0,3),c(0,0,phi_ij))%*%
            rotation_onto_z(atom2_pos_j)+1/L*t(rotation_onto_z(atom1_pos_i))%*%
            cbind(rep(0,3),rep(0,3),c(0,0,phi_ij_prime))%*%
            rotation_onto_z(atom2_pos_j)
        ...
        if(j==1){
          ...
          alpha_ij_prime=-(r_i^2+r_j^2)/(4*sig^2)*4*sigma2
          gamma_ij_prime=-r_i*r_j/(2*sig^2)*4*sigma2
          ...
          K_prime_sigma2=K_prime_sigma2-1.5*(4*sigma2)*((sig)^(-2.5)*(2*pi)^(-1.5))*
            t(rotation_onto_z(atom1_pos_i))%*%cbind(rep(0,3),rep(0,3),c(0,0,phi_ij))%*%
            rotation_onto_z(atom2_pos_j)+1/L*t(rotation_onto_z(atom1_pos_i))%*%
            cbind(rep(0,3),rep(0,3),c(0,0,phi_ij_prime))%*%
            rotation_onto_z(atom2_pos_j)
  ...
  return(list(K_prime_sigma1=K_prime_sigma1,K_prime_sigma2=K_prime_sigma2))
}

cross_glielmo_cov_mat=function(x1,x2,sigma1,sigma2){
  cov= matrix(0, ncol =3 * ifelse(length(as.matrix(x2)) == 9, 1,nrow(x2)),
              nrow = 3 * ifelse(length(as.matrix(x1)) == 9, 
                                1, nrow(x1)))
  ...
  for(i in 1:(length(as.matrix(x1))/9)){
    for (j in 1:(length(as.matrix(x2))/9)){
      if(length(as.matrix(x2)) == 9){
        if(length(as.matrix(x1)) == 9){
          cov1= glielmo_3d_kernel(x1,x2,sigma1,sigma2)
          
        }else{
          cov1= glielmo_3d_kernel(x1[i,],x2,sigma1,sigma2)
          
        }
      }else if(length(as.matrix(x1)) == 9){
        cov1= glielmo_3d_kernel(x1,x2[j,],sigma1,sigma2)
        
      }else{
        cov1= glielmo_3d_kernel(x1[i,],x2[j,],sigma1,sigma2)
...
log_likelihood_glielmo=function(ytr,xtr,sigma1,sigma2){
  if(length(ytr)>3){
    if(ncol(ytr)==3){
      ytr=c(ytr[,1],ytr[,2],ytr[,3])
    }
  ...
  ntr=ifelse(length(xtr)==9,1,nrow(xtr))
  Ktr=cross_glielmo_cov_mat(xtr,xtr,sigma1,sigma2)+
    diag(jit,nrow=3*ntr)
...
grad_glielmo=function(xtr,ytr,sigma1,sigma2){
  Trace=function(x){sum(diag(x))}
  x1=x2=xtr
  norm=function(x){sum(x^2)^.5}
  
  K=cross_glielmo_cov_mat(xtr,xtr,sigma1,sigma2)+
    diag(jit,nrow=3*nrow(xtr))
  ...
  for(i in 1:(length(as.matrix(x1))/9)){
    for (j in 1:(length(as.matrix(x2))/9)){
      x1=xtr[i,]
      x2=xtr[j,]
      deriv=glielmo_3d_kernel_derivative(x1,x2,sigma1,sigma2)
      cov1=deriv$K_prime_sigma1
      cov2=deriv$K_prime_sigma2
      
      x1=x2=xtr
      
      
      
      K_sigma1[i, j] = cov1[1,1]
      ...
      K_sigma1[ifelse(length(as.matrix(x1))==9,1,nrow(x1)) + i, j] = cov1[2,1]
      K_sigma1[2*ifelse(length(as.matrix(x1))==9,1,nrow(x1)) + i, j] = cov1[3,1]
      
      K_sigma1[i,ifelse(length(as.matrix(x2))==9,1,nrow(x2)) + j] = cov1[1,2]
      K_sigma1[i,2*ifelse(length(as.matrix(x2))==9,1,nrow(x2)) + j] = cov1[1,3]
      ...
      K_sigma2[ifelse(length(as.matrix(x1))==9,1,nrow(x1)) + i, j] = cov2[2,1]
      K_sigma2[2*ifelse(length(as.matrix(x1))==9,1,nrow(x1)) + i, j] = cov2[3,1]
      
      K_sigma2[i,ifelse(length(as.matrix(x2))==9,1,nrow(x2)) + j] = cov2[1,2]
      K_sigma2[i,2*ifelse(length(as.matrix(x2))==9,1,nrow(x2)) + j] = cov2[1,3]
  ...
  grad=0.5*c(
    ...
    -t(c(ytr[,1],ytr[,2],ytr[,3]))%*%inv_K%*%K_sigma2%*%inv_K%*%c(
      ytr[,1],ytr[,2],ytr[,3])+Trace(inv_K%*%K_sigma2)
    )
...
glielmo_3d_kernel=function(x1,x2,sigma1,sigma2,w1,w2){
  norm=function(x){sum(x^2)^.5}
  sigma=c(sigma1,sigma2)
  w=c(w1,w2)
  ...
  for(i in 1:2){
    ...
    for( j in 1:2){
      ...
      alpha_ij=(r_i^2+r_j^2)/(4*sig)
      phi_ij=exp(-alpha_ij)/(gamma_ij^2)*(gamma_ij*cosh(gamma_ij)-sinh(gamma_ij))
if((phi_ij==Inf)||is.nan(phi_ij)){
         phi_ij=0
       }      
K=K+w[i]*w[j]/L*t(rotation_onto_z(atom1_pos_i))%*%cbind(rep(0,3),rep(0,3),c(0,0,phi_ij))%*%
        rotation_onto_z(atom2_pos_j)
    }
  }
  return(K)
...
glielmo_3d_kernel_derivative=function(x1,x2,sigma1,sigma2,w1,w2){
  norm=function(x){sum(x^2)^.5}
  ...
  sigma=c(sigma1,sigma2)
  w=c(w1,w2)
  K_prime_sigma1=K_prime_sigma2=K_prime_w1=K_prime_w2=matrix(0,nrow=3,ncol=3)
  for(i in 1:2){
    atom1_pos_i=x1[(3*(i-1)+1):(3*i)]
    r_i=norm(atom1_pos_i)
    for( j in 1:2){
      ...
      if(i==1){
        if(j==1){
          ...
          K_prime_sigma1=K_prime_sigma1-1.5*w[i]*w[j]*(4*sigma1)*((sig)^(-2.5)*(2*pi)^(-1.5))*
            t(rotation_onto_z(atom1_pos_i))%*%cbind(rep(0,3),rep(0,3),c(0,0,phi_ij))%*%
            rotation_onto_z(atom2_pos_j)+1/L*w[i]*w[j]*t(rotation_onto_z(atom1_pos_i))%*%
            cbind(rep(0,3),rep(0,3),c(0,0,phi_ij_prime))%*%
            rotation_onto_z(atom2_pos_j)
          
          K_prime_w1=K_prime_w1+ 2*w1/L*t(rotation_onto_z(atom1_pos_i))%*%cbind(rep(0,3),rep(0,3),c(0,0,phi_ij))%*%
            rotation_onto_z(atom2_pos_j)
          ...
          K_prime_sigma1=K_prime_sigma1- 1.5*w[i]*w[j]*(2*sigma1)*((sig)^(-2.5)*(2*pi)^(-1.5))*
            t(rotation_onto_z(atom1_pos_i))%*%cbind(rep(0,3),rep(0,3),c(0,0,phi_ij))%*%
            rotation_onto_z(atom2_pos_j)+1/L*w[i]*w[j]*t(rotation_onto_z(atom1_pos_i))%*%
            cbind(rep(0,3),rep(0,3),c(0,0,phi_ij_prime))%*%
            rotation_onto_z(atom2_pos_j)
          ...
          K_prime_sigma2=K_prime_sigma2- 1.5*w[i]*w[j]*(2*sigma2)*((sig)^(-2.5)*(2*pi)^(-1.5))*
            t(rotation_onto_z(atom1_pos_i))%*%cbind(rep(0,3),rep(0,3),c(0,0,phi_ij))%*%
            rotation_onto_z(atom2_pos_j)+1/L*w[i]*w[j]*t(rotation_onto_z(atom1_pos_i))%*%
            cbind(rep(0,3),rep(0,3),c(0,0,phi_ij_prime))%*%
            rotation_onto_z(atom2_pos_j)
          
          K_prime_w1=K_prime_w1+ w2/L*t(rotation_onto_z(atom1_pos_i))%*%cbind(rep(0,3),rep(0,3),c(0,0,phi_ij))%*%
            rotation_onto_z(atom2_pos_j)
          K_prime_w2=K_prime_w2+ w1/L*t(rotation_onto_z(atom1_pos_i))%*%cbind(rep(0,3),rep(0,3),c(0,0,phi_ij))%*%
            rotation_onto_z(atom2_pos_j)
        ...
        if(j==1){
          ...
          K_prime_sigma2=K_prime_sigma2- 1.5*(2*sigma2)*w[i]*w[j]*((sig)^(-2.5)*(2*pi)^(-1.5))*
            t(rotation_onto_z(atom1_pos_i))%*%cbind(rep(0,3),rep(0,3),c(0,0,phi_ij))%*%
            rotation_onto_z(atom2_pos_j)+1/L*w[i]*w[j]*t(rotation_onto_z(atom1_pos_i))%*%
            cbind(rep(0,3),rep(0,3),c(0,0,phi_ij_prime))%*%
            rotation_onto_z(atom2_pos_j)
          ...
          K_prime_sigma2=K_prime_sigma2-1.5*(4*sigma2)*w[i]*w[j]*((sig)^(-2.5)*(2*pi)^(-1.5))*
            t(rotation_onto_z(atom1_pos_i))%*%cbind(rep(0,3),rep(0,3),c(0,0,phi_ij))%*%
            rotation_onto_z(atom2_pos_j)+1/L*w[i]*w[j]*t(rotation_onto_z(atom1_pos_i))%*%
            cbind(rep(0,3),rep(0,3),c(0,0,phi_ij_prime))%*%
            rotation_onto_z(atom2_pos_j)
          K_prime_w2=K_prime_w2+ 2*w2/L*t(rotation_onto_z(atom1_pos_i))%*%cbind(rep(0,3),rep(0,3),c(0,0,phi_ij))%*%
            rotation_onto_z(atom2_pos_j)
        }
      }
  ...
  return(list(K_prime_sigma1=K_prime_sigma1,K_prime_sigma2=K_prime_sigma2,
              K_prime_w1=K_prime_w1,K_prime_w2=K_prime_w2))
}


cross_glielmo_cov_mat=function(x1,x2,sigma1,sigma2,w1,w2){
  cov= matrix(0, ncol =3 * ifelse(length(as.matrix(x2)) == 9, 1,nrow(x2)),
              nrow = 3 * ifelse(length(as.matrix(x1)) == 9, 
                                1, nrow(x1)))
  ...
  for(i in 1:(length(as.matrix(x1))/9)){
    for (j in 1:(length(as.matrix(x2))/9)){
      if(length(as.matrix(x2)) == 9){
        if(length(as.matrix(x1)) == 9){
          cov1= glielmo_3d_kernel(x1,x2,sigma1,sigma2,w1,w2)
          
        }else{
          cov1= glielmo_3d_kernel(x1[i,],x2,sigma1,sigma2,w1,w2)
          
        }
      }else if(length(as.matrix(x1)) == 9){
        cov1= glielmo_3d_kernel(x1,x2[j,],sigma1,sigma2,w1,w2)
        
      }else{
        cov1= glielmo_3d_kernel(x1[i,],x2[j,],sigma1,sigma2,w1,w2)
...
log_likelihood_glielmo=function(ytr,xtr,sigma1,sigma2,w1,w2){
  if(length(ytr)>3){
    if(ncol(ytr)==3){
      ytr=c(ytr[,1],ytr[,2],ytr[,3])
    }
  ...
  ntr=ifelse(length(xtr)==9,1,nrow(xtr))
  Ktr=cross_glielmo_cov_mat(xtr,xtr,sigma1,sigma2,w1,w2)+
    diag(jit,nrow=3*ntr)
  
  
  ll=-0.5*t(ytr)%*%inv(Ktr)%*%ytr-0.5*determinant(Ktr)$modulus-ntr*log(2*pi)
  #print(c(ll))
  ll=ifelse(is.nan(ll),10^8,-ll)
  #ll=ifelse(ll>0,ll,1e6)
...
grad_glielmo=function(xtr,ytr,sigma1,sigma2,w1,w2){
  Trace=function(x){sum(diag(x))}
  x1=x2=xtr
  norm=function(x){sum(x^2)^.5}
  
  K=cross_glielmo_cov_mat(xtr,xtr,sigma1,sigma2,w1,w2)+
    diag(jit,nrow=3*nrow(xtr))
  
  K_sigma1=K_sigma2=K_w1=K_w2=
    matrix(0, ncol =3 * ifelse(length(as.matrix(xtr)) == 9, 1, nrow(xtr)),
           nrow = 3 * ifelse(length(as.matrix(xtr)) == 9, 1, nrow(xtr)))
  ...
  for(i in 1:(length(as.matrix(x1))/9)){
    for (j in 1:(length(as.matrix(x2))/9)){
      x1=xtr[i,]
      x2=xtr[j,]
      deriv=glielmo_3d_kernel_derivative(x1,x2,sigma1,sigma2,w1,w2)
      cov1=deriv$K_prime_sigma1
      cov2=deriv$K_prime_sigma2
      cov3=deriv$K_prime_w1
      cov4=deriv$K_prime_w2
      ...
      K_w1[ifelse(length(as.matrix(x1))==9,1,nrow(x1)) + i, j] = cov3[2,1]
      K_w1[2*ifelse(length(as.matrix(x1))==9,1,nrow(x1)) + i, j] = cov3[3,1]
      
      K_w1[i,ifelse(length(as.matrix(x2))==9,1,nrow(x2)) + j] = cov3[1,2]
      K_w1[i,2*ifelse(length(as.matrix(x2))==9,1,nrow(x2)) + j] = cov3[1,3]
      ...
      K_w2[ifelse(length(as.matrix(x1))==9,1,nrow(x1)) + i, j] = cov4[2,1]
      K_w2[2*ifelse(length(as.matrix(x1))==9,1,nrow(x1)) + i, j] = cov4[3,1]
      
      K_w2[i,ifelse(length(as.matrix(x2))==9,1,nrow(x2)) + j] = cov4[1,2]
      K_w2[i,2*ifelse(length(as.matrix(x2))==9,1,nrow(x2)) + j] = cov4[1,3]
...
RMSES=LogS=numeric(ntrain);OPT_PAR=matrix(nrow=ntrain,ncol=4)
...
opt_par=Adam(function(x) {
    log_likelihood_glielmo(
      Ytr,Xtr,x[1],x[2],x[3],x[4])
  },function(x) {
    grad_glielmo(Xtr, Ytr, x[1],x[2],x[3],x[4])
  },c(1,1,2,2),lb=1e-4,lr=lr,max_iter=max_iter)


  OPT_PAR[i,]=opt_par
  Ktetr2=cross_glielmo_cov_mat(Xte,Xtr,
                         opt_par[1],opt_par[2],opt_par[3],opt_par[4])

  Ktetr_trtr_inv2=Ktetr2%*%solve(cross_glielmo_cov_mat(Xtr,Xtr,opt_par[1],opt_par[2],opt_par[3],opt_par[4])+diag(jit,nrow=3*nrow(Xtr)))
  ...
  (RMSES[i]=rmse2=mean(apply((cbind(posterior_mean2[1:(length(posterior_mean)/3)],
  ...
  posterior_cov2= cross_glielmo_cov_mat(Xte,Xte,opt_par[1],opt_par[2],opt_par[3],opt_par[4])-
    Ktetr_trtr_inv2%*%t(Ktetr2)


  (LogS[i]=LogS2=(t(c(Yte[,1],Yte[,2],Yte[,3])-posterior_mean2)%*%
                      solve(posterior_cov2
                            +diag(1e-4,3*nrow(Xte))
                      )%*%
...
save(list = "res", file = paste0("learning_curves_glielmo_full_", id, ".rda"))
