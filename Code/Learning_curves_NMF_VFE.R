...
del_sigma_cross_cov_mat=function(x1, x2, l, sigma) {
 2*sigma*cross_cov_mat(x1, x2, l, 1)
}

del_l_cross_cov_mat=function(x1,x2,l,sigma){
  cov= matrix(0, ncol =3 * ifelse(length(as.matrix(x2)) == 27, 1,nrow(x2)),
              nrow = 3 * ifelse(length(as.matrix(x1)) == 27, 
                                1, nrow(x1)))
  norm=function(x){sum(x^2)^.5}
    ...
    for (j in 1:(length(as.matrix(x2))/27)){
      ...
      if (length(as.matrix(x2)) == 27) {
        if (length(as.matrix(x1)) == 27) {
          ...
          cov1 <- cov_mat(x1[i, ], x2, l, sigma)*norm(x1[i, ]-x2)^2/(l^3)
        }
      } else if (length(as.matrix(x1)) == 27) {
        cov1 <- cov_mat(x1, x2[j, ], l, sigma)*norm(x1-x2[j, ])^2/(l^3)
      } else {
        cov1 <-cov_mat(x1[i, ], x2[j, ], l, sigma)*norm(x1[i, ]-x2[j, ])^2/(l^3)
      }
...
grad_standard=function(xtr,ytr,l,sigma){
  ...
  K=cross_cov_mat(xtr,xtr,l,sigma)+
    diag(jit,nrow=3*ntr)
  K_l=del_l_cross_cov_mat(x1,x2,l,sigma)
  K_sigma=del_sigma_cross_cov_mat(x1,x2,l,sigma)
...
ELBO_standard=function(ytr,xtr,xind,l,sigma,sigma_noise){
  if(length(ytr)>3){
    if(ncol(ytr)==3){
      ytr=c(ytr[,1],ytr[,2],ytr[,3])
    }
  ...
  n=ifelse(length(xtr)==27,1,nrow(xtr))*3
  m=ifelse(length(xind)==27,1,nrow(xind))*3
  ...
  A=Knm%*%Lmm_inv
  elbo=-0.5*(t(ytr)%*%(sigma_noise^(-2)*diag(rep(1,n))-
                         sigma_noise^(-4)*A%*%
                         solve(diag(rep(1,m))+sigma_noise^(-2)*t(A)%*%A)%*%t(A))%*%ytr)%>%as.numeric-
    0.5*log(sigma_noise^(2*n))-
    0.5*determinant(diag(rep(1,m))+sigma_noise^(-2)*t(A)%*%A)$modulus[1]-
    n/2*log(2*pi)-
    (2*sigma_noise^2)^(-1)*(n*sigma^2-sum(diag(Kmm_inv%*%Kmn%*%Knm)))
    
  elbo=ifelse(is.nan(elbo),10^6,-2*elbo)
...
ELBO_fund=function(ytr,xtr,xind,l,sigma,noise_var){
  ...
  n=ifelse(length(xtr)==27,1,nrow(xtr))
  m=ifelse(length(xind)==27,1,nrow(xind))
  Knn=cross_fund_cov_mat(xtr,xtr,l,sigma)
  Knm=cross_fund_cov_mat(xtr,xind,l,sigma)
  Kmn=cross_fund_cov_mat(xind,xtr,l,sigma)
  Kmm=cross_fund_cov_mat(xind,xind,l,sigma)
  Lmm=chol(Kmm)
  Kmm_inv=chol2inv(L)
  Qnn=Knm%*%Kmm%*%Kmn

  
  elbo=-0.5*sum((ytr)*solve(Qnn+diag(rep(sigma^2,n)),ytr))-
        0.5*determinant(Qnn+diag(rep(sigma^2,n)))$modulus[1]-
        n*log(2*pi)-
        (2*sigma^2)^(-1)*sum(diag(Knn-Qnn))

  elbo=ifelse(is.nan(elbo),10^6,-elbo)
...
res=list(RMSES,LogS)
save(list = "res", file = paste0("Results/learning_curves_NMF_VFE_", id, ".rda"))
